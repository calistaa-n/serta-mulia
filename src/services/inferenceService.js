...
async function predictClassification(model, image) {
    try {
        ...
        if (!prediction || typeof prediction.data !== 'function') {
            throw new InputError('Prediksi gagal, respons model tidak valid.');
        }

        // Ambil data prediksi secara asynchronous
        const score = await prediction.data();

        // Pastikan skor tidak kosong
        if (!score || score.length === 0) {
            throw new InputError('Prediksi tidak menghasilkan skor yang valid.');
        }
